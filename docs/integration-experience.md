...
**问题**：在本地开发环境一切正常，但部署到 Vercel 后，点击"立即开始免费使用"按钮无法跳转到 backend 项目的前端页面。

**根本原因**：
- Vercel 只部署单个项目（contract-suite-website）
- Backend 项目的前端文件不在 contract-suite-website 项目目录中
- 本地开发时可能通过其他方式访问了 backend 的前端文件

**正确做法**：
- 将 backend 项目的前端页面文件（index.html 及其依赖）复制到 contract-suite-website 的 public 目录
- 确保所有需要的文件都在同一个项目中
- 部署前在本地测试构建后的版本（`npm run build && npm run preview`）

### 2. 需求理解的误差成本

**初始理解**：需要将两个独立项目（contract-suite-website 和 backend）进行复杂的整合，包括路由配置、状态共享、API 集成等。

**实际需求**：只是简单地从 landing page 跳转到另一个 HTML 页面。

**解决方案**：将 backend 的前端页面文件放到 contract-suite-website 项目的 public 目录下即可。

**教训**：
- 🎯 **先明确核心需求**：不要假设需求的复杂度，多次确认
- 🎯 **从简单方案开始**：先尝试最简单的解决方案，再考虑复杂方案
- 🎯 **避免过度设计**："从 A 跳转到 B" 可能只是文件放置问题，不一定需要复杂的架构
- 🎯 **沟通要具体**：用具体的文件路径、URL、操作步骤来描述需求，减少误解
...
1. **⚠️ Vercel 部署的特殊性**
   - **问题**：在 Vercel 部署时，只显示 contract-suite-website 的 landing page，无法跳转到 backend 项目对应的前端页面
   - **原因**：Vercel 默认只部署一个项目，不会自动包含其他项目的文件
   - **解决方案**：将 backend 项目的前端页面文件（index.html）直接放置在 contract-suite-website 项目的 public 目录下
   - **教训**：部署前要考虑生产环境的文件结构，不能假设多个项目会自动整合

2. **💡 需求理解的重要性**
   - **初始误解**：以为需要复杂的项目整合、路由配置、状态共享等
   - **实际需求**：只是简单地从 landing page 跳转到另一个 HTML 页面
   - **真实解决方案**：将 backend 项目的前端页面文件（index.html）放到 contract-suite-website 项目目录下即可
   - **教训**：
     - 先明确核心需求，避免过度设计
     - 简单的问题往往有简单的解决方案
     - 多次确认需求，减少沟通误差
     - "从 A 项目跳转到 B 项目" 不一定需要复杂的整合，可能只是文件放置问题

3. **依赖检查至关重要**
   - 不要只复制 HTML 文件，要检查所有依赖
   - 使用工具自动化检测外部资源
   - 特别注意 `<script src="...">` 引用的 JavaScript 文件
...
**版本历史**：
- v1.1 (2026-02-05): 添加 Vercel 部署经验和需求理解误差的核心教训
